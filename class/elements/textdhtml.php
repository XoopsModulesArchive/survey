...
###############################################################################
/**
 * Class for Dhtml Textarea Elements
 * @author Jan Pedersen
 * @copyright copyright (c) 2004 IDG.dk
 * @package Survey
 * @subpackage Elements
 */
class DhtmlTextareaElement extends XoopsFormDhtmlTextArea {
    var $_question;
    var $canBeRequired = true;
    var $canBeSaved = true;
    var $hasMultipleAnswers = false;
    function DhtmlTextareaElement(&$question) {
        ...
        $ele_value[0] = $myts->stripSlashesGPC($ele_value[0]);
        $ele_value[0] = $myts->htmlSpecialChars($ele_value[0]);

        $this->XoopsFormDhtmlTextArea(
    ...
    */
    function getValueForSave($ele_value) {
        ...
        if( intval($ele_value[1]) != 0 ){
            $value[1] = intval($ele_value[1]);
        }else{
            $value[1] = $xoopsModuleConfig['ta_rows'];
        }
        if( intval($ele_value[2]) != 0 ){
            $value[2] = intval($ele_value[2]);
        }else{
            $value[2] = $xoopsModuleConfig['ta_cols'];
        }
        return $value;
    }

    /*
    * Returns a text string for insertion of answer into the database
    * @param string $reply
    *
    * @return string
    */
    ...
    */
    function getAdminElement($options = array()) {
        ...
        $default = new XoopsFormDhtmlTextArea(_AM_SV_ELE_DEFAULT, 'ele_value[0]', $default_value, 10, 35);

        $return = array($rows, $cols, $default);
        $return[] = new XoopsFormHidden('ele_type', 'DhtmlTextarea');
        return $return;
    }
}
?>